...
resource "aws_cloudfront_origin_access_control" "react-app" {
...
resource "aws_cloudfront_distribution" "react-app" {
  ...
  origin {
    domain_name = aws_s3_bucket.react-app.bucket_regional_domain_name
    origin_id = local.s3_origin_id
    origin_access_control_id = aws_cloudfront_origin_access_control.react-app.id
  }
  restrictions {
    geo_restriction {
